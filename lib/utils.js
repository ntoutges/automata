...
export class UnclampedRGB {
    ...
    add(other) {
        return new UnclampedRGB(this.r + other.r, this.g + other.g, this.b + other.b);
    }
    sub(other) {
        return new UnclampedRGB(this.r - other.r, this.g - other.g, this.b - other.b);
    }
...
export function isMonochromeRGBWithinRange(rgbTest, rgbA, rgbB) {
    const minR = Math.min(rgbA.r, rgbB.r);
    const maxR = Math.max(rgbA.r, rgbB.r);
    const minG = Math.min(rgbA.g, rgbB.g);
    const maxG = Math.max(rgbA.g, rgbB.g);
    ...
    if (rgbTest.r < minR || rgbTest.r > maxR
        || rgbTest.g < minG || rgbTest.g > maxG
        || rgbTest.b < minB || rgbTest.b > maxB) {
        return false;
    }
...
export class Draggable {
    ...
    lockY;
    el;
    constructor({ el, onClick = () => { }, onUnClick = () => { }, onRender = () => { }, lockX = false, lockY = false, scale = 40, doScroll = false }) {
        this.el = el;
