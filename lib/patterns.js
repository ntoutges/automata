...
export class PatternRange extends QuantumPattern {
    ...
    equals(other) {
        if (other instanceof CollapsedPattern) { // check if all RGB values within range
            ...
            if (otherColors.width != width || otherColors.height != height)
                return false; // different dimensions = difference
...
export class PatternRangeMonochrome extends PatternRange {
    equals(other) {
        if (other instanceof CollapsedPattern) { // check if all RGB values within range
            ...
            for (let x = 0; x < width; x++) {
                for (let y = 0; y < height; y++) {
                    if (!isMonochromeRGBWithinRange(other.getColor(x, y), patternA.getColor(x, y), patternB.getColor(x, y)))
                        return false; // component outside range, therefore doesn't match
                }
            }
            return true;
            ...
            if (other instanceof PatternRangeMonochrome)
                return this.getPatternA().equals(other.getPatternA()) && this.getPatternB().equals(other.getPatternB());
            return false; // not the same type of pattern, so immediatly different
        }
    }
    collapse() {
        ...
        for (let x = 0; x < width; x++) {
            for (let y = 0; y < height; y++) {
                colors.setAt(generateMonochromeRGBWithinRange(patternA.getColor(x, y), patternB.getColor(x, y), modifier), x, y);
            }
        }
        return new Pattern(colors);
    }
...
export class PatternAddative extends QuantumPattern {
    ...
    equals(other) {
        if (other instanceof CollapsedPattern) { // check if all RGB values within range
            ...
            if (other instanceof PatternAddative)
    ...
    collapse({ oldPattern }) {
        ...
        for (let x = 0; x < width; x++) {
            for (let y = 0; y < height; y++) {
                colors.setAt(oldPattern.getColor(x, y).add(this.pattern.getColor(x, y)), x, y);
            }
        }
        return new Pattern(colors);
    }
...
export class PatternSubtractive extends QuantumPattern {
    ...
    equals(other) {
        if (other instanceof CollapsedPattern) { // check if all RGB values within range
            ...
            if (other instanceof PatternSubtractive)
                return this.pattern.equals(other.getPattern());
            return false; // not the same type of pattern, so immediatly different
        }
    }
    collapse({ oldPattern }) {
        ...
        for (let x = 0; x < width; x++) {
            for (let y = 0; y < height; y++) {
                colors.setAt(oldPattern.getColor(x, y).sub(this.pattern.getColor(x, y)), x, y);
            }
        }
        return new Pattern(colors);
    }
...
export class PatternKeyed extends QuantumPattern {
    ...
    fallback;
    lastKey;
    constructor(pattern, fallbackKey = null) {
        super();
        if (fallbackKey && !(fallbackKey in pattern))
            throw new Error(`Invalid fallback value--"${fallbackKey}" does not exist in the set of patterns`);
        ;
    ...
    getPattern(key) { return this.patterns[key]; }
    // checks if all other pattern values >= this pattern value
    equals(other) {
        if (other instanceof CollapsedPattern) {
            // loop through all possibilities
            for (const key in this.patterns) {
                if (this.patterns[key].equals(other))
                    return true;
            }
            return false;
        }
            ...
            if (other instanceof PatternKeyed) {
                ...
                if (thisKeys.length != otherKeys.length)
                    return false; // different amount of keys, definitely not the same
                for (let i in thisKeys) {
                    if (thisKeys[i] != otherKeys[i])
                        return false; // keys different
    ...
    getXInverted() {
        ...
        for (let key in this.patterns) {
            patterns[key] = this.patterns[key].getXInverted();
        }
